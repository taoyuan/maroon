...
var CONFIG_FNS = ['get', 'set', 'setAll', 'enabled', 'disabled', 'enable', 'disable', 'configure'];
...
function Maroon(root, app, options) {
   ...
   this.options = options = options || {};
    ...
    if (app) {
        ...
        if (options.forward) {
            var functions;
            if (typeof options.forward === 'string') {
                functions = [options.forward];
            } else if (Array.isArray(options.forward)) {
                functions = options.forward;
            } else {
                functions = CONFIG_FNS;
            }
            utils.forward(app, maroon, functions);
        }
    }
...
_.assign(Maroon.prototype, configurable);
    ...
    fs.readdirSync(dir).forEach(function(file) {
        ...
        if (stats.isFile()) {
            ...
            if ('function' === typeof conf) {
                ...
                maroon.set(basename, conf[maroon.set('env')]);
            }
        }
    });
};
...
Maroon.prototype.applyArgv = function () {
    var argv = this.options.argv;
    if (argv) {
        if (this.app && this.app.set && this.app.settings) {
            for (var key in argv) if (argv.hasOwnProperty(key)) this.app.set(key, argv[key]);
        } else {
            this.setAll(argv);
        }
    }
