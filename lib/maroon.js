...
Maroon.prototype.env = function (val) {
    if (arguments.length === 0) {
        if (isConfigurable(this.app)) return this.env();
        return this.conf.get('env');
    }
    if (isConfigurable(this.app)) return this.app.set('env', val);
    return this.conf.set('env', val);
};
